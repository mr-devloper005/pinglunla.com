...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  return (
    <section className="mt-14 border-t border-[var(--tk-line)] pt-10">
      ...
      <form onSubmit={submit} className="mt-6 rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 shadow-[0_16px_40px_rgba(19,78,142,0.08)]">
        <input
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="Your name (optional)"
          maxLength={60}
          className="h-12 w-full rounded-full border border-[var(--tk-line)] bg-[var(--tk-bg)] px-5 text-sm font-semibold text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <textarea
          value={text}
          ...
          rows={3}
          maxLength={1500}
          className="mt-3 w-full resize-y rounded-[24px] border border-[var(--tk-line)] bg-[var(--tk-bg)] px-5 py-4 text-sm leading-6 text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
      ...
      <div className="mt-6 grid gap-3">
        {all.map((comment) => (
          <div key={comment.id} className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 shadow-[0_12px_30px_rgba(19,78,142,0.06)]">
